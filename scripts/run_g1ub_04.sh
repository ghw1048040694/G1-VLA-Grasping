...
OUTPUT_DIR="$PROJECT_ROOT/outputs/G1UB-04_Genesis_saturation_audit"
...
ENV_COUNTS=(768 1024 1280)

bash "$PROJECT_ROOT/scripts/setup_g1ub_genesis.sh"
mkdir -p "$OUTPUT_DIR"

SUMMARY="$OUTPUT_DIR/capacity_summary.csv"
printf 'num_envs,status,duration_seconds,peak_gpu_mib,peak_wsl_used_mib,peak_gpu_util_percent,peak_gpu_power_w,peak_gpu_temp_c,exit_code\n' > "$SUMMARY"

for num_envs in "${ENV_COUNTS[@]}"; do
  log_file="$OUTPUT_DIR/num_envs_${num_envs}.log"
  telemetry_file="$OUTPUT_DIR/num_envs_${num_envs}_gpu.csv"
  start_time=$(date +%s)
  peak_gpu=0
  peak_ram=0
  peak_util=0
  peak_power=0
  peak_temp=0
  printf 'timestamp,gpu_util_percent,gpu_memory_mib,gpu_power_w,gpu_temp_c\n' > "$telemetry_file"

  echo "Starting 10-iteration Genesis saturation probe: num_envs=$num_envs"
  (
    cd "$ASAP_ROOT"
    ...
    "$PYTHON" humanoidverse/train_agent.py \
      ...
      project_name=G1UB-04 \
      experiment_name="saturation_${num_envs}" \
  ...
  while kill -0 "$train_pid" 2>/dev/null; do
    IFS=, read -r gpu_util gpu_used gpu_power gpu_temp < <(
    ...
    printf '%s,%s,%s,%s,%s\n' "$(date --iso-8601=seconds)" "$gpu_util" "$gpu_used" "$gpu_power" "$gpu_temp" >> "$telemetry_file"
    (( gpu_used > peak_gpu )) && peak_gpu=$gpu_used
    (( ram_used > peak_ram )) && peak_ram=$ram_used
    (( gpu_util > peak_util )) && peak_util=$gpu_util
    peak_power=$(awk -v a="$peak_power" -v b="$gpu_power" 'BEGIN {print (b>a)?b:a}')
    (( gpu_temp > peak_temp )) && peak_temp=$gpu_temp
    sleep 1
  done
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$num_envs" "$status" "$duration" "$peak_gpu" "$peak_ram" \
    "$peak_util" "$peak_power" "$peak_temp" "$exit_code" | tee -a "$SUMMARY"
