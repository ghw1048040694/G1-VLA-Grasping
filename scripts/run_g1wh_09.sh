...
OUTPUT_DIR="$PROJECT_ROOT/outputs/G1WH-09_contact_pair_attribution"
...
"$PYTHON" "$PROJECT_ROOT/scripts/validate_g1_bimanual_actuation.py" \
  ...
  --gain-profile unitree \
  --dynamics-profile both \
  --enable-gravity \
  --enable-contacts \
  --base-mode model \
  --experiment-name G1WH-09-contact-pair-attribution
