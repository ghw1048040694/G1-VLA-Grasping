...
from g1_language_action_adapter import ADAPTER_NAME, attach_language_action_adapter
...
def configure_trainable_parameters(
    policy, unfreeze_vlm_text_layers: int, freeze_action_path: bool
...
def update_policy_with_shared_flow(
    ...
    with (
        ...
        loss, output = policy.forward(dict(batch), noise=noise, time=time_values)
    correct_per_sample = output["losses_after_rm_padding"].mean(dim=(1, 2)).detach()
    ...
    total_loss = loss + target_classification_weight * target_loss
    ...
    return tracker.vlm_grad_norm.val
...
def main() -> None:
    ...
    trainable_audit, monitored_parameters = configure_trainable_parameters(
        policy, args.unfreeze_vlm_text_layers, args.freeze_action_path
    )
    ...
    if args.verify_update_only:
        ...
        vlm_grad_norm = update_policy_with_shared_flow(
            tracker,
            policy,
            batch,
            optimizer,
        ...
        if monitored_parameters and vlm_grad_norm <= 0.0:
            raise RuntimeError("Selected VLM text layers received no gradient")
