...
"$PYTHON" "$PROJECT_ROOT/scripts/evaluate_g1wh_smolvla_closed_loop.py" \
  ...
  --hybrid "baseline_to_recovery3000=baseline500,recovery3000" \
  --experiment-id G1WH-36-phase-routed-recovery-policy \
  ...
  --device cuda \
  --output-dir "$PROJECT_ROOT/outputs/G1WH-36_phase_routed_recovery_policy"
