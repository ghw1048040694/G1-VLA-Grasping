...
BASELINE_OUTPUT="$PROJECT_ROOT/outputs/G1WH-29_phase_smolvla_1000step"
RECOVERY_OUTPUT="$PROJECT_ROOT/outputs/G1WH-34R_recovery_smolvla_3000step"
...
"$PYTHON" "$PROJECT_ROOT/scripts/evaluate_g1wh_smolvla_closed_loop.py" \
  ...
  --checkpoint "baseline500=$BASELINE_OUTPUT/checkpoints/000500/pretrained_model" \
  --checkpoint "recovery1000=$RECOVERY_OUTPUT/checkpoints/001000/pretrained_model" \
  --checkpoint "recovery2000=$RECOVERY_OUTPUT/checkpoints/002000/pretrained_model" \
  --checkpoint "recovery3000=$RECOVERY_OUTPUT/checkpoints/003000/pretrained_model" \
  --experiment-id G1WH-35-recovery-checkpoint-closed-loop-evaluation \
  ...
  --device cuda \
  --output-dir "$PROJECT_ROOT/outputs/G1WH-35_recovery_checkpoint_closed_loop_evaluation"
