...
OUTPUT_ROOT="$PROJECT_ROOT/outputs/G1WH-38_hold_gain_ablation"
...
run_condition() {
  ...
  "$PYTHON" "$PROJECT_ROOT/scripts/evaluate_g1wh_smolvla_closed_loop.py" \
    ...
    --skip-standalone \
    --experiment-id "G1WH-38-hold-gain-ablation-${name}" \
    ...
    --hold-gain-scale "$gain_scale" \
    --device cuda \
    --output-dir "$OUTPUT_ROOT/$name"
}

run_condition boosted_gain100 1.0
run_condition nominal_gain067 0.67
