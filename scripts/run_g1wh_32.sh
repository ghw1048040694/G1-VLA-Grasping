...
"$PYTHON" "$PROJECT_ROOT/scripts/evaluate_g1wh_smolvla_closed_loop.py" \
  ...
  --policy-selection step500 \
  --experiment-id G1WH-32-moderate-hold-action-smoothing \
  ...
  --hold-action-blend-alpha 0.40 \
  --device cuda \
  --output-dir "$PROJECT_ROOT/outputs/G1WH-32_moderate_hold_action_smoothing"
